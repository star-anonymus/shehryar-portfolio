...
export const projects: Project[] = [
  {
    ...
    summary:
      "Deaf-first emergency dispatch: three Flutter apps and a React portal on a Node.js + TypeScript API.",
    description:
      "Final year project. Patient, responder and caregiver apps in Flutter plus a React admin portal, all on one Node.js and TypeScript server — Express routes with a Socket.io room per emergency, so the live map, the ETA and the control room never disagree.",
    details: [
      "The server is layered rather than flat: 16 route groups behind JWT auth, role checks, rate limits and validation, then 14 controllers that only shape requests, 17 services that hold the business rules — dispatch, escalation, privacy, calls, safeguards — and 10 repositories where every Prisma query lives.",
      "Push is self-hosted through a background service instead of Firebase, so emergency location and medical data never leave the stack. Medical fields are encrypted at rest in PostgreSQL, and access is split by role across patient, responder, caregiver and admin.",
      "The patient app is built deaf-first: one-tap SOS, visual alerts and quick phrases rather than a call. Responders get a Gemini-generated medical summary with route and ETA from Google Directions, and can switch to WebRTC voice or chat once they accept.",
    ],
    highlights: [
      "One-tap SOS that sends location and medical profile together",
      "A Socket.io room per emergency — patient, responder, caregiver and admin share one live map",
      "Self-hosted push through a background service, not Firebase",
      "Gemini AI medical summary, with route and ETA from Google Directions",
      "RBAC over access and refresh tokens; medical fields encrypted at rest",
      "WebRTC calling and chat once a responder accepts",
      "React admin portal: responder verification, live SOS map, call records, audit logs",
    ],
    tags: [
      "Flutter",
      "Riverpod",
